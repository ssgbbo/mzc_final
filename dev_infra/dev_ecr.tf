...
resource "aws_ecr_repository" "dev-lsb-ecr" {
  name                 = "dev-lsb-ecr"
...
resource "aws_ecr_lifecycle_policy" "ecr_policy" {
  repository = aws_ecr_repository.dev-lsb-ecr.name
...
locals {
  ...
  depends_on = [aws_ecr_repository.dev-lsb-ecr]
}
...
locals {
  ...
  docker_build_command = "docker build -t ${aws_ecr_repository.dev-lsb-ecr.name} ./dev_app"
  docker_tag_command   = "docker tag ${aws_ecr_repository.dev-lsb-ecr.name}:latest ${local.account_id}.dkr.ecr.${var.region}.amazonaws.com/${aws_ecr_repository.dev-lsb-ecr.name}:latest"
  docker_push_command  = "docker push ${local.account_id}.dkr.ecr.${var.region}.amazonaws.com/${aws_ecr_repository.dev-lsb-ecr.name}:latest"
}
